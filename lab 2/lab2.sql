...
CREATE TABLE "EventCategory" (
    ...
    CONSTRAINT uq_eventcategory_pair UNIQUE (event_id, category_id)
);
...
CREATE TABLE "EventParticipant" (
    ...
    CONSTRAINT uq_participant_pair UNIQUE (event_id, user_id)
);
...
CREATE TABLE "EventMention" (
    ...
    CONSTRAINT fk_eventmention_user
        FOREIGN KEY (user_id) REFERENCES "User"(id) ON DELETE CASCADE,
    CONSTRAINT uq_eventmention_pair UNIQUE (event_id, user_id)
);
...
CREATE TABLE "CommentMention" (
    ...
    CONSTRAINT fk_commentmention_user
        FOREIGN KEY (user_id) REFERENCES "User"(id) ON DELETE CASCADE,
    CONSTRAINT uq_commentmention_pair UNIQUE (comment_id, user_id)
);
...
VALUES
    (1,2,'active'), (1,3,'active'), (2,1,'active'),
    (3,4,'blocked'), (4,5,'active');

INSERT INTO "EventParticipant" (event_id, user_id, status)
VALUES
    (1,1,'going'), (1,2,'interested'), (2,3,'going'),
    (3,4,'declined'), (4,5,'going');

INSERT INTO "Comment" (event_id, user_id, text)
VALUES
    (1,1,'Чекаю на доповіді про AI!'),
    (1,2,'Слайди будуть доступні після події.'),
    (2,3,'Хто приєднається до проекту?'),
    (3,4,'Які артисти виступатимуть?'),
    (4,5,'Команда готова до турніру!');

INSERT INTO "EventMention" (event_id, user_id)
VALUES (1,3), (2,1), (3,5), (4,2);
